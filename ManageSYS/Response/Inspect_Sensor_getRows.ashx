...
public class Inspect_Sensor_getRows : IHttpHandler {
    ...
    public string createSql3(bool Isteamgroup, string prodCode, string startTime, string endTime, string teamcode, string timecode)
    {
        ...
        string query = "select inspect_code,inspect_name from ht_qlt_inspect_proj  where inspect_code in (select r.inspect_code from ht_qlt_inspect_proj r left join ht_inner_inspect_group s on s.id = r.inspect_group left join ht_qlt_inspect_stdd_sub t on t.inspect_code = r.inspect_code and t.is_del = '0' where r.inspect_group = '4' and r.is_del = '0')  and is_del = '0' order by inspect_code";
